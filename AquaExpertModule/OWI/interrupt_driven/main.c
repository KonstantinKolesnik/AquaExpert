 ...
 *  connected to the 1-Wire bus. The temperature sensor is continuously 
...
void OWI_StateMachine()
{
    ...
    // If an error has occurred since last time, clear all flags and return to idle state.
    if (OWIStatus.error)
    {
    ...
    switch (state)
    {
        ...
        case OWI_STATE_DETECT_PRESENCE1:
        {
            ...
            // If presence was detected, send Skip ROM and Start conversion signals. 
            else
            {
                buf[0] = OWI_ROM_SKIP;
        ...
        {
            // Receive one byte of data to check for completion of the temperature conversion.
        ...
        {
            // If everything received was zero. Jump to the last state to receive a new byte.
            if (buf[0] == 0x00)
            {
                state = OWI_STATE_WAIT_FOR_CONVERSION1;
            }
            // If there was at least 1 one received, continue with a new reset.
            else
            {
                OWI_DetectPresence();
        ...
        case OWI_STATE_DETECT_PRESENCE2:
        {
            ...
            // If presence was detected, send Skip ROM and Read scratchpad signals. 
            else
            {
                buf[0] = OWI_ROM_SKIP;
        ...
        {
            // Compare the computed crc with the crc read from the scratchpad. 
            crc = 0;
            for(i = 0; i < 8; i++)
            {
            ...
            // If they don't match, go back to the second Reset to read the scratchpad again.
            else
            {
                OWI_DetectPresence();
                state = OWI_STATE_DETECT_PRESENCE2;
