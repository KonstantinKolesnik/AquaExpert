...
IPAddress TCP_IP(192, 168, 1, 177);				// Configure your static ip-address here

//uint8_t TCP_MAC[] = { 0x02, 0xDE, 0xAD, 0x00, 0x00, 0x42 };	// Mac-address - You should change this! see note *2 above!
uint8_t TCP_MAC[] = { 0xF2, 0xDE, 0xAD, 0x00, 0x00, 0x42 }; //x2, x6, xA or xE (replace x with any hex value) for using Local Ranges.
...
void onEthernetMessage()
{
	...
	if (client)
	{
		...
		Serial.print("<<");
		char buf[4];
		for (uint8_t a = 0; a < inputSize; a++)
			sprintf(buf, "%02X ", (uint8_t)inputString[a]); Serial.print(buf);
...
void onRadioMessage(const char *writeBuffer, uint8_t *writeSize)
{
	...
	Serial.print(">>");
	char buf[4];
	for (uint8_t a = 0; a < *writeSize; a++)
	{
		sprintf(buf, "%02X ", (uint8_t)writeBuffer[a]);
		Serial.print(buf);
	}
	Serial.println("");
#endif
